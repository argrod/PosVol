...
hcat(sum(ismissing.(Dat.Depth) .== false),sum(ismissing.(Dat.Pitch) .== false),sum(ismissing.(Dat.Roll) .== false),sum(ismissing.(Dat.Speed) .== false),sum(ismissing.(Dat.Heading) .== false))
...
begin
	md"""
	## Identify the vector of the animal
	
	Taking the heading, pitch, and roll information, the heading can be plotted. The speed can then indicate the vector length.
	"""
end
